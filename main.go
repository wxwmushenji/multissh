...
func main() {
	...
	Interaction := flag.Bool("I", false, "Interaction")
	...
	if *cfgFile == "" {
		for _, host := range hostList {
			...
			host_Struct.Interaction = *Interaction
	...
	limitFunc := func(chLimit chan bool, ch chan g.SSHResult, host g.SSHHost) {
		funcs.Dossh(host.Username, host.Password, host.Host, host.Key, host.CmdList, host.Port, *timeLimit, cipherList, host.LinuxMode, host.Interaction, ch)
		<-chLimit
	}
