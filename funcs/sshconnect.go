...
func Dossh(username, password, host, key string, cmdlist []string, port, timeout int, cipherList []string, linuxMode bool, Interaction bool, ch chan g.SSHResult) {
	chSSH := make(chan g.SSHResult)
	if linuxMode {
		go dossh_run(username, password, host, key, cmdlist, port, cipherList, chSSH)
	} else if Interaction {
		go dossh_Interaction(username, password, host, key, cmdlist, port, cipherList, chSSH)
	} else {
...
func dossh_Interaction(username, password, host, key string, cmdlist []string, port int, cipherList []string, ch chan g.SSHResult) {
	session, err := connect(username, password, host, key, port, cipherList)
	var sshResult g.SSHResult
	sshResult.Host = host
